...
def fetch_and_store():
    logger.info("开始抓取 paimon.moe 数据")
    try:
        resp = requests.get(URL, timeout=TIMEOUT)
        resp.raise_for_status()
        data = resp.json()
    except requests.exceptions.Timeout:
        logger.warning("paimon.moe 超时")
        print("[fetch_data] 网络超时：paimon.moe 无响应")
        return 0
    except requests.exceptions.ConnectionError:
        logger.warning("paimon.moe 连接失败")
        print("[fetch_data] 网络连接失败，请检查网络")
        return 0
    except requests.exceptions.HTTPError as e:
        logger.warning(f"paimon.moe HTTP错误: {e}")
        print(f"[fetch_data] HTTP 错误：{e}")
        return 0
    except Exception as e:
        logger.exception(f"paimon.moe 未知错误: {e}")
