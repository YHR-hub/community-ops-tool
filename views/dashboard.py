...
class DashboardMixin:
    ...
    def _build_input_tab(self, parent):
        ...
        # CSV 导入
        import_row = ctk.CTkFrame(frame, fg_color=MHY_CARD, corner_radius=8, border_width=1, border_color=MHY_BORDER)
        import_row.pack(fill="x", pady=(0, 15))
        ctk.CTkLabel(import_row, text="📥 批量导入", font=("Microsoft YaHei", 12, "bold"),
                      text_color="white").pack(anchor="w", padx=10, pady=(8, 2))
        ctk.CTkLabel(import_row, text="支持 CSV 格式: 日期,游戏,DAU,新增帖子,评论数,平均停留,互动率",
                      font=("Microsoft YaHei", 10), text_color=MHY_SUB, wraplength=650).pack(anchor="w", padx=10)
        ctk.CTkButton(import_row, text="📂 导入 CSV", fg_color="#8b5cf6", hover_color="#7c3aed",
                       command=self._import_csv_data, width=100, height=28).pack(side="left", padx=10, pady=(5, 10))
        ctk.CTkButton(import_row, text="📋 下载模板", fg_color=MHY_BORDER, hover_color="#3a3a4e",
                       text_color=MHY_TEXT, command=self._download_csv_template, width=100, height=28).pack(side="left", padx=5, pady=(5, 10))
    ...
    def _import_csv_data(self):
        fp = filedialog.askopenfilename(filetypes=[("CSV", "*.csv")], title="选择 CSV 数据文件")
        if not fp:
            return
        try:
            count = 0
            with open(fp, "r", encoding="utf-8-sig") as f:
                ...
                if not header:
                    messagebox.showwarning("提示", "CSV 文件为空")
                    return
                with get_conn() as c:
                    for row in reader:
                        if len(row) < 7:
                            continue
                        try:
                            date, game, dau, posts, comments, session, rate = row[0], row[1], int(row[2]), int(row[3]), int(row[4]), float(row[5]), float(row[6])
                            c.execute("""INSERT OR REPLACE INTO daily_metrics
                                         (date,game,dau,new_posts,comments,avg_session,interaction_rate)
                                         VALUES (?,?,?,?,?,?,?)""",
                                       (date, game, dau, posts, comments, session, rate))
                            count += 1
                        except:
                            continue
            self._toast(f"成功导入 {count} 条数据")
            self._save_log("CSV导入", f"{count}条")
        except Exception as e:
            messagebox.showerror("错误", f"导入失败: {e}")

    def _download_csv_template(self):
        fp = filedialog.asksaveasfilename(defaultextension=".csv", filetypes=[("CSV", "*.csv")],
                                           title="下载CSV模板", initialfile="运营数据模板.csv")
        if not fp:
            return
        with open(fp, "w", newline="", encoding="utf-8-sig") as f:
            ...
            w.writerow(["2025-01-02", "原神", "82000", "520", "2100", "13.0", "5.5"])
        self._toast("模板已下载")
    ...
    def _refresh_dash(self, parent):
        ...
        try:
            ...
            # 趋势预测
            self._build_trend_prediction(self.dash_container, daus)
    ...
    def _build_trend_prediction(self, parent, daus):
        prediction = self._predict_dau(daus)
        pred_frame = ctk.CTkFrame(parent, fg_color=MHY_CARD, corner_radius=8,
                                   border_width=1, border_color=MHY_BORDER)
        pred_frame.pack(fill="x", pady=(15, 10))
        ctk.CTkLabel(pred_frame, text="📈 趋势预测",
                      font=("Microsoft YaHei", 14, "bold"), text_color="white").pack(anchor="w", padx=15, pady=(10, 3))
        ctk.CTkLabel(pred_frame, text=prediction,
                      font=("Microsoft YaHei", 13), text_color=MHY_SUB,
                      wraplength=650).pack(anchor="w", padx=15, pady=(0, 10))

    def _predict_dau(self, daus):
        if len(daus) < 3:
            return "数据不足（需至少3天数据），无法预测"
        n = len(daus)
        xs = list(range(1, n + 1))
        ys = daus
        sum_x = sum(xs)
        ...
        next_val = round(slope * (n + 1) + intercept)
        last_val = ys[-1]
        change_pct = (next_val - last_val) / last_val * 100 if last_val else 0
        direction = "↑ 上升" if change_pct > 1 else ("↓ 下降" if change_pct < -1 else "→ 稳定")
        return f"基于最小二乘法线性回归，明日预计 DAU：{next_val:,} ({direction}，变化 {change_pct:+.1f}%)\n基于最近 {n} 天数据 | 当前值: {last_val:,} | 预测值: {next_val:,}"
